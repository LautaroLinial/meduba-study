    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push("/")}
            className="px-6 py-3 bg-sky-500 rounded-lg text-white hover:bg-sky-600 transition-colors"
          >
            Volver al inicio
          </button>
        </div>
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "⚠️ Hubo un error al procesar tu pregunta. Por favor intentá de nuevo. Si el error persiste, verificá que la API key esté configurada correctamente.",
        },
      ]);
    } finally {
      setIsLoading(false);
      ...
      <header className="border-b border-slate-800/50 px-4 h-14 flex items-center justify-between bg-slate-950/80 backdrop-blur-xl shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-slate-300">
            {materia.icon} {materia.name} — {yearData.name}
          </span>
        </div>
        <button
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((msg, i) => (
              ...
              >
                ...
                >
                  {msg.role === "assistant" && (
                    <div className="text-[11px] text-sky-400 font-semibold tracking-wider mb-1.5">
                      TUTOR MEDUBA
                    </div>
                  )}
                  <div
          ...
          <div className="border-t border-slate-800/50 p-4 bg-slate-950/50 shrink-0">
                ...
                className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg shrink-0 transition-all
                  ${
                    input.trim() && !isLoading
                      ? "bg-gradient-to-r from-sky-500 to-indigo-500 text-white cursor-pointer hover:shadow-lg hover:shadow-sky-500/20"
            ...
            <p className="text-center text-[11px] text-slate-600 mt-2">
              Respuestas basadas en: {materia.libros.map((l) => l.split(" - ")[0]).join(" · ")}
            </p>
          </div>
        </div>
        ...
        {sidebarOpen && (
          <div className="w-72 border-l border-slate-800/50 bg-black/20 overflow-y-auto shrink-0 hidden md:block">
            <div className="p-5">
              {/* Bibliografía */}
              <h3 className="text-[11px] text-slate-500 tracking-[2px] uppercase font-semibold mb-3">
                Bibliografía
              </h3>
              {materia.libros.map((libro) => (
                <div
                  key={libro}
                  className="flex items-center gap-2 p-2.5 mb-2 rounded-lg bg-sky-500/[0.06] border border-sky-500/10 text-slate-400 text-sm"
                >
                  <span>📖</span>
                  <span>{libro.split(" - ")[0]}</span>
                </div>
              ))}

              {/* Temas */}
              <h3 className="text-[11px] text-slate-500 tracking-[2px] uppercase font-semibold mt-6 mb-3">
                Temas
              </h3>
              ...
              <div className="mt-6 p-3.5 bg-indigo-500/[0.06] border border-indigo-500/10 rounded-xl">
                ...
                <p className="text-slate-500 text-xs leading-relaxed">
                  Hacé preguntas específicas para mejores respuestas. Ej:
                  &quot;Describí la inervación del túnel carpiano según
                  Latarjet&quot;
                </p>
              </div>
            </div>
